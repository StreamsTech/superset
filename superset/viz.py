...
### Custom BaseDeckGLViz classes
class CustomBaseDeckGLViz(BaseViz):
...
class DeckScatterVizExtend(CustomBaseDeckGLViz):
...
class FormViz(BaseViz):
    ...
    viz_type = "form_chart"
    verbose_name = "Form Chart"
    is_timeseries = False

    def get_data(self, df):
        # Extract fields from frontend form data
        form_data = self.form_data
        all_columns = form_data.get("all_columns", [])
        user_inputs = json.loads(form_data.get("user_inputs", "{}"))
        table_name = self.datasource.table_name
        database = self.datasource.database

        if not all_columns or not user_inputs:
            return {"status": "error", "message": "Missing columns or user inputs."}

        # Generate the INSERT SQL
        columns_sql = ", ".join(f"`{col}`" for col in all_columns)
        values_sql = ", ".join(f"'{user_inputs[col]}'" for col in all_columns)
        insert_sql = f"INSERT INTO `{table_name}` ({columns_sql}) VALUES ({values_sql});"

        # Execute query (optional)
        if form_data.get("submit", False):
            database.get_sqla_engine().execute(insert_sql)

        return {
            "status": "success",
            "insert_sql": insert_sql,
            "message": "Row inserted successfully." if form_data.get("submit") else "SQL generated.",
        }
