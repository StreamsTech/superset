...
export const getColorFunctionExtend = (
  {
    operator,
    targetValue,
    targetValueLeft,
  ...
  return (value: number) => {
    ...
    if (alpha === undefined || alpha) {
      return addAlpha(
        colorScheme,
        getOpacityExtend(value, cutoffValue, extremeValue, minOpacity, maxOpacity),
      );
    }
    return colorScheme;
  };
...
export const getColorFormattersExtend = memoizeOne(
  (
    columnConfig: ConditionalFormattingConfigExtend[] | undefined,
    data: DataRecord[],
    alpha?: boolean,
  ) =>
    columnConfig?.reduce(
      (acc: ColorFormattersExtend, config: ConditionalFormattingConfigExtend) => {
        if (
          ...
          acc.push({
            solidColor: config?.solidFill,
            column: config?.column,
            getColorFromValue: getColorFunctionExtend(
              config,
