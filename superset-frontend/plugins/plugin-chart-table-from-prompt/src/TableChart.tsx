...
export default function TableChart<D extends DataRecord = DataRecord>(
  ...
  const handleDropdownChange = (value: string) => {
    ...
    // Optionally reset fields on change
    setDimension('');
    setMetric('');
  };
    ...
    try {
      const response = await fetch(`/api/v1/chart/${chartId}`, {
        ...
        headers: {
          ...
          'X-CSRFToken': getCsrfToken(), // you need to get this from cookies
        },
        credentials: 'include',
      });
      ...
      if (response.ok) {
        ...
        //window.location.href = '/chart/list'; // Or redirect user to a safe location
  ...
  const handleChartSubmit = async (values: { columns: string }) => {
    if (selectedChart !== 'pie_extend') return;

    // Step 1: Parse dimension (groupby) string into array
    const groupby = dimension
      .split(',')
      .map(col => col.trim())
      .filter(Boolean);

    // Step 2: Parse metric string like "count[col5]" into object
    const metricMatch = metric.match(/^(\w+)\[(.+)\]$/);
    if (!metricMatch) {
      Modal.error({ title: 'Invalid metric format', content: 'Use format like count[col5]' });
      return;
    }

    const aggregate = metricMatch[1].toUpperCase();
    const column = metricMatch[2];
    ...
    const res = await fetch('/prompt_table/create_viz', {
      ...
      body: JSON.stringify({
        viz_type: selectedChart,
        groupby: groupby,
        metric: metricObj,
        dataset_id: formData.datasource.split('__')[0],
        dashboard_id: parseInt(dashboardIdFromURL ?? '0', 10),
        slice_id: chartId,
      }),
    });
  ...
  return (
    <div style={{ overflowX: 'auto', overflowY: 'auto', height, width }}><Styles>
      <button
      ...
      <div><button onClick={() => setShowModal(true)}>View Available charts</button></div>
      {showModal && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <select onChange={(e) => handleDropdownChange(e.target.value)}>
              ...
              <option value="pie_extend">Pie Chart</option>
              <option value="echarts_timeseries_bar_extend">Bar Chart</option>
              <option value="table_extend">Table</option>
              <option value="big_number_extended">Big Number</option>
            </select>
            {/* Conditionally show extra inputs if pie_extend is selected */}
            {selectedChart === 'pie_extend' && (
              <div style={{ marginTop: '1rem' }}>
                <div>
                  <label style={{ textAlign: 'left', display: 'block' }}>Dimension (comma-separated columns):</label>
                  <input
                    type="text"
                    value={dimension}
                ...
                <div style={{ marginTop: '0.5rem' }}>
                  <label style={{ textAlign: 'left', display: 'block' }}>Metric:</label>
                  <input
                    type="text"
                    value={metric}
                    onChange={(e) => setMetric(e.target.value)}
                    placeholder="e.g. count[country]"
                    style={{ width: '100%' }}
                  />
                </div>
              </div>
            ...
            <button
              ...
              onClick={() => handleChartSubmit({ columns: dimension })}
            >
              Submit
            </button>
          </div>
