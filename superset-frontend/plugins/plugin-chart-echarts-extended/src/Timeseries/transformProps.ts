...
import { getDefaultTooltip, tooltipPercentage, tooltipValueSummation } from '../utils/tooltip';
  ...
  const echartOptions: EChartsCoreOption = {
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        const tooltipSummation = tooltipValueSummation(forecastValues);
        Object.keys(forecastValues).forEach(key => {
          ...
          const percentValue = value.observation && tooltipPercentageShow ? tooltipPercentage(value.observation, tooltipSummation) : '';
          const contentStyle =
            key === focusedSeries ? 'font-weight: 700' : 'opacity: 0.7';
          rows.push(`<span style="${contentStyle}">${content} ${percentValue}</span>`);
        });
        if (stack) {
          rows.reverse();
        }
