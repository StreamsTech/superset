...
const ColumnColorFormatingControl = ({
  value,
  onChange,
  columnOptions,
  verboseMap,
  ...
  const [conditionalFormattingConfigs, setConditionalFormattingConfigs] =
    useState<ColumnColorFormatingConfig[]>(value ?? []);
  ...
  const onSave = (config: ColumnColorFormatingConfig) => {
    setConditionalFormattingConfigs(prevConfigs => [...prevConfigs, config]);
  };

  const onEdit = (newConfig: ColumnColorFormatingConfig, index: number) => {
...
export default ColumnColorFormatingControl;
